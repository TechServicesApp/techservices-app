...
export const envoiportfolio = async ({ idsender, id,date }) => {
  try {
    const portfolioEnvoye = doc(db, "DemandeServicesBureau", id);
...
export const tousBesoins = async () => {
  const devisQuery = query(
    collection(db, "DemandeServicesBureau"),
    where("cibleCollection", "==", "ServiceEntrepriseBureau"),
  ...
  besoins.forEach((devis) => {
    ...
    if (devis.selected ===localStorage.getItem("quincailler")) {
      ...
      if (!besoinFound) {
        addDoc(notifEnvoye, {
          iddoc: devis.id,
          lien: `/visualisationbesoin_bureau/${devis.id}`,
          type: "besoinsaccepte",
