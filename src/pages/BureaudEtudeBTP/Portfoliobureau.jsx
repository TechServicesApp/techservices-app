...
const profile = () => {
  ...
  const fetchpost = async () => {
    try {
      ...
      setnombre(nbre);
      const uid =localStorage.getItem("quincailleruid");
      const specialite = await getDoc(
        doc(db, "ServiceEntrepriseBureau", localStorage.getItem("idspecialite"))
      );

      if (uid.length > 0) {
        const q1 = query(
          collection(db, "BureaudEtudeBTP"),
          where("uid", "==", uid)
        );
            ...
            <div className="lg:col-span-4 col-span-12">
              <Card title="Info">
                <ul className="list space-y-8">
                  ...
                  <li className="flex space-x-3 rtl:space-x-reverse">
                    <div className="flex-none text-2xl text-slate-600 dark:text-slate-300">
                      <Icon icon="material-symbols:auto-awesome-motion-rounded" />
                    </div>
                    <div className="flex-1">
                      <div className="uppercase text-xs text-slate-500 dark:text-slate-300 mb-1 leading-[12px]">
                        NUMERO AGREMENT
