...
const FormWizard = () => {
	...
	const specialiteQuincaillerieRef = collection(db, "ServicesEntrepriseBTP");
	...
	const entreprisebtp = collection(db, "EntrepriseBtp");
	...
	const onSubmit = async (data, e) => {
		...
		if (isLastStep) {
			try {
				...
				const idquincailler =await 	addDoc(entreprisebtp, {
					//ajouter les informations de entreprise dans firebasee
					doc1: doc1,
					doc2: doc2,
					logo: logo,
					email: email,
					...
					longitude: longitude,
					latitude: latitude,
					idservice: "dlJINMSa04nPdbhH56Oy",
					idspecialite: value2,
					idoption: valuespecialite,
					collectionSpecialite: "ServicesEntrepriseBTP",
					validationStatus: 0,
					uid: user.uid,
					agrement: agrement,
					annee: annee,
				...
				await sendNotifAdmin({newDocId:idquincailler.id,message:"Nouvelle inscription",type:"EntrepriseBtp"}).then(()=>{console.log("notif admin envoye")})
...
>
  <div className="space-y-4">
    ...
    <div className="mb-4">
      <p className="text-sm font-bold">Avez-vous un numéro d'ordre ?</p>
    ...
    {/* Étape 3 : Si l'utilisateur n'a pas de numéro d'ordre */}
    {hasOrderNumber === "non" && (
      <p className="text-sm text-gray-600 dark:text-gray-400">
        Continuez avec l'étape d'inscription.
      </p>
    )}
  </div>
</Card>
