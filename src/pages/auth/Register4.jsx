...
const personalSchema = yup.object().shape({
	annee: yup.number().required().required("veuillez remplir"),
});
...
const FormWizard = () => {
	...
	const specialiteQuincaillerieRef = collection(db, "ServiceEntrepriseBureau");
	...
	const entreprisebtp = collection(db, "BureaudEtudeBTP");
		...
		if (isLastStep) {
			try {
				...
				const idquincailler = await addDoc(entreprisebtp, {
					...
					motpasse: password,
					nom: nom,
					entrepriseUid: user.uid,
					telephone: telephone,
					longitude: longitude,
					latitude: latitude,
					idservice: "5rUVXFsgMNr2lTQebSKa",
					idspecialite: value2,
					idoption: valuespecialite,
					collectionSpecialite: "ServiceEntrepriseBureau",
					validationStatus: 0,
					uid: user.uid,
					agrement: agrement,
					annee: annee,
							...
							{stepNumber === 1 && (
								<div>
									<div className="col">
										...
										<Fileinput
											className="row"
											label="Joindre un dossier fiscal"
											type="file"
