...
import { Ajoutannonce, Afficherannonce } from "./fonctions/Fonctionsannonces";
import { v4 } from "uuid";
...
const Annonce = () => {
	...
	const handleUpload2 = (e) => {
		...
		const reflogo = ref(
			imgdb,
			`annonceentreprise/${new Date().getTime() + v4()}`
		);
	...
	const onSubmit = async (data) => {
		try {
			const q1 = query(
				collection(db, 	localStorage.getItem("idservice") === "21xHEmO1HQC4Hst4QNup"
        ? "Architecture"
        ...
        : "EntrepriseBtp"),
			...
			const idquincailler = await Ajoutannonce({
				auteur:
