...
export const DateEvenement = async ({ datejour, id }) => {
  try {
    ...
    // Normalisation de la date actuelle pour comparaison
    const normalizedCurrentDate = normalizeDate(new Date());
   

    // Filtrer les documents dont la date `start` correspond à la date actuelle
    const filteredDates = querySnapshot.docs
      .map((doc) => ({
        id: doc.id,
        ...doc.data(),
      }))
      .filter((doc) => {
        const startDate = doc.start ? new Date(doc.start.seconds*1000) : null;
        console.log(startDate)
        return startDate && normalizeDate(startDate).getTime() === normalizedCurrentDate.getTime();
      });
